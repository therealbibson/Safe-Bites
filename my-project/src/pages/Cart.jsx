      ...
      <main className="max-w-3xl mx-auto px-4 py-8">
        {loading ? (
          <div className="text-center py-20">
            <p className="text-2xl text-stone-400 font-bold">Loading your cart...</p>
          </div>
        ) : cart.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-[2.5rem] shadow-sm border border-stone-100 mt-8">
            ...
            <Link to="/" className="bg-orange-500 text-white px-10 py-4 rounded-full font-black uppercase tracking-widest text-sm shadow-lg shadow-orange-100">
              Browse Menu
            </Link>
          </div>
        ) : (
          <div className="space-y-4 mt-8">
            {cart.map((item) => (
              <div key={item.productId} className="bg-white p-5 rounded-[2rem] flex items-center shadow-sm border border-stone-50 hover:shadow-md transition-all">
                <img src={item.imageUrl} alt={item.name} className="w-24 h-24 object-cover rounded-2xl mr-6" />
                <div className="flex-1">
                  <h3 className="font-black text-stone-800 text-lg leading-tight">{item.name}</h3>
                  <p className="text-stone-500 text-sm mb-1">Qty: {item.quantity}</p>
            ...
            <div className="mt-12 bg-white p-8 rounded-[2.5rem] shadow-sm border border-stone-100">
              ...
              <button 
                ...
                className="block text-center w-full bg-orange-500 text-white py-6 rounded-[2rem] font-black text-xl shadow-xl shadow-orange-200 hover:bg-orange-600 transition-all uppercase tracking-widest"
              >
                Proceed to Checkout
              </button>
            </div>
